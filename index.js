...
//nocache
nocache('./ryuu.js', module => console.log(color('|TRM|'), color(`${module} Updated!`, 'cyan')))
...
const starts = async (kurr = new WAConnection()) => {
	...
	console.log(color(figlet.textSync('RyuuBase', {
		font: 'Standard',
...
console.log(color('Jangan Dijual/belikan Script Ini','cyan'), color(`\nWatermark By Ryuu\n`,'silver'))
	kurr.browserDescription = [ 'Ryuu', 'safari', '3.0' ]
	...
	try {
		...
		if (anu.action == 'add') {
			...
			try {
				...
				teks = `- [ WELCOME ] - \n\nHi @${num.split('@')[0]}\n\n*${mdata.subject}* Selalu membuka pintu untuk siapa saja`
			...
			try {
				...
				teks = `- [ LEAVING ] -\n\nBye bye@${num.split('@')[0]}\n\n*${mdata.subject}* selalu menanti ke hadiranmu`
